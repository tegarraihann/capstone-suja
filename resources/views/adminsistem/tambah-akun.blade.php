...
@section('content')
    ...
    <div class="w-full p-5 h-full">
        <a class="font-medium text-2xl" href="{{ url('adminsistem/dashboard') }}"><i
                class="fa-solid fa-angle-left text-xl"></i> Tambah Akun</a>
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-4 bg-white mt-5">
            <form class="mx-auto" action="{{ url('adminsistem/dashboard/tambah-akun') }}" method="post">
                {{ csrf_field() }}
                <div class="mb-5">
                ...
                <div class="mb-5">
                    ...
                    <select id="role" name="role"
                        ...
                        @foreach ($roleOptions as $option)
                            <option {{ old('role') == $option['value'] ? 'selected' : '' }}
                                value="{{ $option['value'] }}">
                                {{ $option['label'] }}</option>
                        @endforeach
                    </select>
                ...
                <button type="submit"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Buat
                    akun</button>
            </form>
        </div>
